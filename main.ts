...
namespace PlanetX_AILens {
    ...
    */
    ...
    export function ClearlearnObject(): void {
        let thingsBuf2 = pins.createBuffer(9)
        thingsBuf2[0] = 10
        thingsBuf2[1] = 10
        pins.i2cWriteBuffer(CameraAdd, thingsBuf2)
    }
    /**
...
namespace GigoWorkshop {
    ...
    */
    ...
    export function motorStop2(channel: GgMotorChannel): void {
        ...
        pins.digitalWritePin(dirPin2, 0);
    ...
    export function ping(trig: DigitalPin, echo: DigitalPin, unit: PingUnitgigo, maxCmDistance = 500): number {
        ...
        switch (unit) {
            case PingUnitgigo.Centimeters: return Math.idiv(e, 58);
            case PingUnitgigo.Inches: return Math.idiv(e, 148);
            default: return e;
        }
    }
    ////////////////////////////////
...
namespace GigoSensor {
    ...
    export function temperature(temperature_origin: MLX90614_TEMPERATURE_ORIGIN): number {
        let u = read_reg_uint16(temperature_origin);
        return u * 0.02 - 273.15;
    }
    ...
    export function ping(channel: SonarPort, unit: PingUnitold, maxCmDistance = 500): number {
        let trig2 = TrigChanelold[channel];
        let echo2 = EchoChanelolde[channel];
        // send pulse
        ...
        // read pulse
        const o = pins.pulseIn(echo2, PulseValue.High, maxCmDistance * 58);

        switch (unit) {
            case PingUnitold.Centimeters: return Math.idiv(o, 58);
            case PingUnitold.Inches: return Math.idiv(o, 148);
            default: return o;
        }
    }
    ...
    export function sensorping(trig: DigitalPin, echo: DigitalPin, unit: PingUnitold, maxCmDistance = 500): number {
        ...
        // read pulse
        const p = pins.pulseIn(echo, PulseValue.High, maxCmDistance * 58);

        switch (unit) {
            case PingUnitold.Centimeters: return p / 58;
            case PingUnitold.Inches: return p / 148;
            default: return p;
        }
    }
    //% group="ect"
    ...
    export function PIR(p: SensorChannel): boolean {
        let q = SensorChannels[p];
        let a: number = pins.digitalReadPin(q);
        if (a == 1) {
            return true;
        } else return false;
    }
...
namespace GigoMotor {
    ...
    export function motorControl2(channel: MotorChannel, direction: MotorShaftDirection, speed: number): void {
        ...
        pins.digitalWritePin(dirPin6, direct);
